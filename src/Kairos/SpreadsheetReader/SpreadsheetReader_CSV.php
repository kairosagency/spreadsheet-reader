...
class SpreadsheetReader_CSV implements \Iterator, \Countable
{
    ...
    private $isISOFile = false;

    private $isMACFile = false;
    ...
    public function next()
    {
        ...
        if ($this -> CurrentRow)
        {
            ...
            $convertMethod = $this->getConvertMethod();
            $Encoding = $this -> Encoding;
            foreach ($this -> CurrentRow as $Key => $Value)
            {
                if($convertMethod == 'mb') {
                    $this -> CurrentRow[$Key] = trim(trim(mb_convert_encoding($Value, 'UTF-8'), $this->Options['Enclosure']));
                }
                elseif($convertMethod == 'iconv') {
                    $this -> CurrentRow[$Key] = trim(trim(iconv('MACINTOSH', 'UTF-8', $Value), $this->Options['Enclosure']));
                }
                /*$this -> CurrentRow[$Key] = trim(trim(
                    $this->getConvertValue($Value),
    ...
    /**
     * Check if the file is in ISO-8859-1 and then if is MACINTOSH encoded
     * If the file is MACINTOSH encoded we send iconv in order to use inconv else we send mb in order to use mb_convert_encoding
     * Because mb_convert_encoding does not support MACINTOSH encoding
     *
     * We also make this check only on 20 lines after what we consider that we dont have to check (can be changed but maybe perf issue ?)
     */
    public function getConvertMethod()
    {
        if($this->isMACFile === true) {
            return 'iconv';
        }

        // if file is ISO-8859-1 encoded file
        if($this->isISOFile === true && $this->Index <= 20) {

            preg_match_all('/(83|88|8d|8e|8f|90|96|9d)/i', bin2hex(implode('', $this->CurrentRow)), $matches, PREG_OFFSET_CAPTURE);

            if(count($matches[0]) > 0) {
                foreach($matches[0] as $ele) {
                    if(isset($ele[1]) && $ele[1]%2 == 0) {
                        $this->isMACFile = true;
                        return 'iconv';
                    }
                }
